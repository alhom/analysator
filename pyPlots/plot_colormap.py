...
def plot_colormap(filename=None,
    ...
    [XmeshXY,YmeshXY] = scipy.meshgrid(np.linspace(simext[0],simext[1],num=int(sizes[0]+1)),np.linspace(simext[2],simext[3],num=int(sizes[1]+1)))
    ...
    if streamlines!=None:
        ...
        if zsize==1:
            ...
            V = slinemap[:,:,2]
        ax1.streamplot(XmeshCentres,YmeshCentres,U,V,linewidth=0.5*streamlinethick, density=streamlinedensity, color=streamlinecolor, arrowsize=streamlinethick*0.3)
